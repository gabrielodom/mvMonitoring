...
# Why don't we have 10080 - 2880 rows in the Alarm matrix?? We need to set the
# simulation run so that this mspMonitor function crawls over all 10080 rows.
# Right now, the code does not "learn" after each run. At each updateFreq, we
# need the function to append the non-alarmed test data to the training data,
# and then rerun mspMonitor on the next updateFreq observations.
```
