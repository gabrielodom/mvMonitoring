...
source("Simulation_Data/Rotation_Playground.R")
...
normal_df %<>%
  mutate(state2 = rep(rep(c(0, 1, 0), each = 60), (24 / 3) * 7)) %>% 
  mutate(state3 = rep(rep(c(0, 0, 1), each = 60), (24 / 3) * 7))
...
state1_df <- normal_df %>%
  filter(state2 == 0) %>% 
  filter(state3 == 0) %>% 
  select(dateTime, state2, state3, x, y, z)
state2_df <- normal_df %>%
  filter(state2 == 1) %>% 
  select(dateTime, state2, state3, x = xState2, y = yState2, z = zState2)
state3_df <- normal_df %>%
  filter(state3 == 1) %>% 
  select(dateTime, state2, state3, x = xState3, y = yState3, z = zState3)
...
fault1A_df <- normal_df %>% 
  select(dateTime, state2, state3, t, x, y, z) %>% 
  mutate(x = x + shift,
         y = y + shift,
         z = z + shift)
...
fault3A_df <- normal_df %>% 
  select(dateTime, state2, state3, t, err1, err2, err3) %>% 
...
library(xts)
normal_switch_xts <- normal_switch_df %>% select(-dateTime)
rownames(normal_switch_xts) <- normal_switch_df[,1]

# Fault 1A
normal_w_fault1A_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                              fault1A_switch_df[faultStart:omega,])
fault1A_xts <- normal_w_fault1A_df %>% select(-dateTime)
rownames(fault1A_xts) <- normal_switch_df[,1]
                   

# Fault 1B
normal_w_fault1B_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                                 fault1B_switch_df[faultStart:omega,])
fault1B_xts <- normal_w_fault1B_df %>% select(-dateTime)
rownames(fault1B_xts) <- normal_switch_df[,1]

# Fault 2A
normal_w_fault2A_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                                 fault2A_switch_df[faultStart:omega,])
fault2A_xts <- normal_w_fault2A_df %>% select(-dateTime)
rownames(fault2A_xts) <- normal_switch_df[,1]

# Fault 2B
normal_w_fault2B_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                                 fault2B_switch_df[faultStart:omega,])
fault2B_xts <- normal_w_fault2B_df %>% select(-dateTime)
rownames(fault2B_xts) <- normal_switch_df[,1]

# Fault 3A
normal_w_fault3A_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                                 fault3A_switch_df[faultStart:omega,])
fault3A_xts <- normal_w_fault3A_df %>% select(-dateTime)
rownames(fault3A_xts) <- normal_switch_df[,1]

# Fault 3B
normal_w_fault3B_df <- bind_rows(normal_switch_df[1:(faultStart - 1),],
                                 fault3B_switch_df[faultStart:omega,])
fault3B_xts <- normal_w_fault3B_df %>% select(-dateTime)
rownames(fault3B_xts) <- normal_switch_df[,1]
...
monitor(D = faults_ls$normal[,3:5],
        method = "PCA",
        train.obs.num = 3 * 24 * 60,
        train.update.freq = 3 * 60,
        alpha = 0.001,
        rm.outlier = FALSE,
        rm.flagged = TRUE,
        thrsh.type = "np",
