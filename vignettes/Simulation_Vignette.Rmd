...
laggedTestObs <- stats::lag(zoo(laggedTestObs), 0:-2)
laggedTestObs <- xts::xts(laggedTestObs[-(1:2),])
laggedTestObs <- cbind(faults_ls$normal[10075:10080, 1], laggedTestObs)
...
```{r testRealData}
laggedOneDay <- stats::lag(zoo::zoo(oneDay_clean), 0:-1)
laggedOneDay[1,36:70] <- tenDay_clean[nrow(tenDay_clean),]
laggedOneDay <- xts::xts(laggedOneDay)
