...
# doesn't work.
a <- Sys.time()
msadResults2_ls <- lapply(1:500, function(void){
...
# hours for 500 iterations.
```
The SPE statistic for the AD-PCA monitoring scheme never rings an alarm for Fault 1A. Interesting.

Let's clean up these results.
```{r msadSim_Cleaning}
FA_rates2 <- lapply(1:length(msadResults2_ls), function(i){
  msadResults2_ls[[i]]$falseAlarmRates
}) %>% do.call(bind_rows, .)
write_csv(FA_rates2, path = "false_alarm_rates_20170314.csv")
detectionTimes2 <- lapply(1:length(msadResults2_ls), function(i){
  msadResults2_ls[[i]]$time2Detect
}) %>% do.call(bind_rows, .)
write_csv(detectionTimes2, path = "detection_times_20170314.csv")
```

Now we can look at the results.
```{r msadSim_graphics}
# False Alarm Rates
FA_rates2[,1] %>% hist(xlim = c(0, 0.05), main = "MSAD SPE False Alarm Rates")
FA_rates[,2] %>% hist(xlim = c(0, 0.05), main = "MSAD T2 False Alarm Rates")
FA_rates[,3] %>% hist(xlim = c(0, 0.05), main = "AD SPE False Alarm Rates")
FA_rates2[,4] %>% hist(xlim = c(0, 0.05), main = "AD T2 False Alarm Rates")
