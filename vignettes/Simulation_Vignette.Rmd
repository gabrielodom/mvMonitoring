...
library(xts)
a <- Sys.time()
# msadResults2_ls <- lapply(1:2, function(void){
msadResults2_ls <- mclapply(1:1000, mc.cores = 8L, function(void){
  ...
  faultsSimRes <- lapply(msadpcaObs_ls[-1], function(mat){
    ...
    # SPE
    time2SPE_msad <- which(msad_MS_Alarm[1000:2580,15] %in% c(1,3))
    time2SPE_msad <- suppressWarnings(min(time2SPE_msad))
    # T2
    time2T2_msad <- which(msad_MS_Alarm[1000:2580,15] %in% c(2,3))
    ...
    ## Time to first alarm
    # SPE
    time2SPE_ad <- which(ad_MS_Alarm[1000:2580,15] %in% c(1,3))
    time2SPE_ad <- suppressWarnings(min(time2SPE_ad))
    # T2
    time2T2_ad <- which(ad_MS_Alarm[1000:2580,15] %in% c(2,3))
    ...
    do.call(bind_rows, .) %>% 
    mutate(Faults = names(adpcaObs_ls[-1]))
...
# iterations over 8 cores.
```
Let's clean up these results.
```{r msadSim_Cleaning}
ms_FA_rates2 <- lapply(1:length(msadResults2_ls), function(i){
  msadResults2_ls[[i]]$falseAlarmRates
}) %>% do.call(bind_rows, .)
write_csv(ms_FA_rates2, path = "false_alarm_rates_ms_1000_20170314.csv")
ms_detectionTimes2 <- lapply(1:length(msadResults2_ls), function(i){
  msadResults2_ls[[i]]$time2Detect
}) %>% do.call(bind_rows, .)
write_csv(ms_detectionTimes2, path = "detection_times_ms_1000_20170314.csv")
```

Now we can look at the results.
```{r msadSim_graphics}
# False Alarm Rates
ms_FA_rates2[,1] %>% hist(xlim = c(0, 0.05), main = "MSAD SPE False Alarm Rates")
ms_FA_rates2[,2] %>% hist(xlim = c(0, 0.05), main = "MSAD T2 False Alarm Rates")
ms_FA_rates2[,3] %>% hist(xlim = c(0, 0.05), main = "AD SPE False Alarm Rates")
ms_FA_rates2[,4] %>% hist(xlim = c(0, 0.05), main = "AD T2 False Alarm Rates")
...
library(xts)
a <- Sys.time()
# adResults_ls <- lapply(1:50, function(void){
adResults_ls <- mclapply(1:1000, mc.cores = 8L, function(void){
  ...
  # Subset out the normal operating data
  faultsSimRes <- lapply(adpcaObs_ls[-1], function(mat){
...
# hours for 1000 iterations over 8 cores.
```

Let's clean up these results.
```{r msadSim_Cleaning}
ss_FA_rates <- lapply(1:length(adResults_ls), function(i){
  adResults_ls[[i]]$falseAlarmRates
}) %>% do.call(bind_rows, .)
write_csv(ss_FA_rates, path = "false_alarm_rates_ss_1000_20170315.csv")
ss_detectionTimes <- lapply(1:length(adResults_ls), function(i){
  adResults_ls[[i]]$time2Detect
}) %>% do.call(bind_rows, .)
write_csv(ss_detectionTimes, path = "detection_times_ss_1000_20170315.csv")
```
